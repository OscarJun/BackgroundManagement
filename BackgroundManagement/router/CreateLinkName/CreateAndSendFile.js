    ...
    if (req.body.LinkName == '') {
        ...
        routeSql.FileLinkName.findOne({where:{LinkName:req.body.LinkName,IsDeleted:false}}).then(function(data){
    		if (data) {
                ...
                routeSql.FileLinkName.create({LinkName:req.body.LinkName,LinkUrl:Ipconfig.Local.LocalIpHost+ ':'+Ipconfig.Local.LocalIpPort+'/CreateLinkFiles/'+req.body.LinkName+'.html',Desc:req.body.Desc}).then(function(){
                    var HtmlUrl = './www/CreateLinkFiles/' + req.body.LinkName + '.html';
